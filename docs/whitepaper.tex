...
To distribute a message, we encrypt its content by a new temporal symmetric key distributed with every message, encrypted by the temporal public key. I add two new fields for a private message: \textbf{nextTempPublicKey} and \textbf{cryptKey}.
\begin{enumerate}
\item For a node $P$ and a message with content $C$.
\item Generate a new temporal public/private keypair for the next message, and set the nextTempPublicKey field to this temporal public key signed by $P$'s private key.
\item Set cryptKey to the randomly generated symmetric key, and sign it with $P$'s current temporal public key. For $P$'s first message, the temporal public key is the initial public key that $P$ generated when starting the conversation. For any message following, the temporal public key is the public key mentioned in the nextTempPublicKey field of the last message. 
...
% TODO recommendInvite

% TODO protocol for video/audio chat
